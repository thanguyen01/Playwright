...
- Name: playground.spec.js >> Playground - Employee Table >> should display employee table with initial data
- Location: tests/playground.spec.js:59:3
...
Locator:  locator('[data-testid="employees-table"]')
Expected: visible
Received: hidden
Timeout:  15000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="employees-table"]')
    34 × locator resolved to <table id="employees-table" data-testid="employees-table">…</table>
       - unexpected value "hidden"

```

```yaml
- heading "🎯 Playwright Playground" [level=1]
...
```ts
  ...
  2   | 
  3   | // Use local playground for reliable testing
  4   | const PLAYGROUND_URL = 'file://' + process.cwd() + '/public/playground.html';
  5   | 
  6   | test.describe('Playground - Login Form', () => {
  7   |   test.beforeEach(async ({ page }) => {
  8   |     await page.goto(PLAYGROUND_URL);
  9   |   });
  10  | 
  11  |   test('should display login form', async ({ page }) => {
  12  |     await expect(page.locator('[data-testid="username-input"]')).toBeVisible();
  13  |     await expect(page.locator('[data-testid="password-input"]')).toBeVisible();
